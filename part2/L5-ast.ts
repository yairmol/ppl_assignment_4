...
export interface LetValuesExp {tag: "LetValuesExp"; bindings: TupleBinding[]; body: CExp[]}
export const makeLetValuesExp = (bindings: TupleBinding[], body: CExp[]): LetValuesExp =>
...
const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> => 
    ...
    safe2((bdgs: TupleBinding[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
        (mapResult(parseTupleBinding, bindings), mapResult(parseL5CExp, body));
...
export const unparse = (e: Parsed): Result<string> =>
    ...
    isProgram(e) ? bind(mapResult(unparse, e.exps), (exps: string[]) => makeOk(`(L5 ${exps})`)) :
    makeFailure("illegal unparsing");
