// L5-typecheck
// ========================================================
import { equals, map, zipWith, reduce } from 'ramda';
...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeNonEmptyTupleTExp, isNonTupleTExp, isTupleTExp, isNonEmptyTupleTExp } from "./TExp";
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> => 
    isPrimOp(app.rator) && app.rator.op === 'values' ? bind(mapResult((rand) => typeofExp(rand, tenv), app.rands), 
        (t: TExp[]) => allT(isNonTupleTExp, t) ? makeOk(makeNonEmptyTupleTExp(t)) : makeFailure("cant have a tuple inside a tuple")) :
...
export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    ...
    const constraints = zipWithResult((varTEs: TExp[], val: AppExp | VarRef) => bind(typeofExp(val, tenv),
        ...
        varTEss, vals);
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(vars, reduce((acc: TExp[], curr: TExp[]) => acc.concat(curr), [], varTEss), tenv)));
};
