...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return (pos === -1)  ? bind(parseTupleTExp(texps), (tes: TExp[]) => 
        allT(isNonTupleTExp, tes) ? makeOk(makeNonEmptyTupleTExp(tes)) : makeFailure("tuple can't be inside another tuple")) :
