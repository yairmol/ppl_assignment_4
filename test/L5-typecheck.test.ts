...
describe('L5 Type Checker', () => {
    ...
    describe('L5typeof', () => {
        ...
        it('returns the type of "values" expression', () => {
            expect(L5typeof("(values 1 2 3)")).to.deep.equal(makeOk("(number * number * number)"));
            expect(L5typeof(`(values 1 "str" #t 3)`)).to.deep.equal(makeOk("(number * string * boolean * number)"));
        });

        it('returns the type of "let-values" expression', () => {
            expect(L5typeof("(let-values ((((x : number) (y : number)) (values 1 2)) (((z : number) (w : number)) (values 3 4))) (+ x y))")).to.deep.equal(makeOk("number"));
        });
